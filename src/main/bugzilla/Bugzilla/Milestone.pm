...
sub set_invisible { $_[0]->set('invisible', $_[1]); }
...
sub bug_count_opened {
    my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{'bug_count_opened'}) {
        $self->{'bug_count_opened'} = $dbh->selectrow_array(q{
            SELECT COUNT(*) FROM bugs
            WHERE product_id = ? AND target_milestone = ? AND bug_status in ('NEW','ASSIGNED','REOPENED')},
            undef, $self->product_id, $self->name) || 0;
    }
    return $self->{'bug_count_opened'};
}
...
sub invisible  { return $_[0]->{'invisible'};  }
...
=head1 SYNOPSIS
    ...
    my $invisible  = $milestone->invisible;

    my $milestone = Bugzilla::Milestone->create(
        { name => $name, product => $product, sortkey => $sortkey, invisible =>$invisible });
...
=item C<sortkey()>
 ...
 Description: Invisible indicator of the milestone.

 Params:      none.

 Returns:     The Invisible indicator of the milestone.
...
=item C<set_sortkey($new_sortkey)>
 ...
 =item C<set_invisible($new_invisible)>

 Description: Changes the invisible indicator of the milestone.

 Params:      $new_invisible - new invisible indicator of the milestone (boolean).
...
=item C<create({name => $name, product => $product, sortkey => $sortkey, invisible => $invisible })>
 ...
 Params:      $name    - name of the new milestone (string). This name
              ...
              $invisible - invisible indicator (boolean)
