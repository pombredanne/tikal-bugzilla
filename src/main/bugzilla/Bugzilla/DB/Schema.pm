...
use constant ABSTRACT_SCHEMA => {
    ...
    bugs => {
        FIELDS => [
            ...
            ### Tikal fields
            entity  			=> {TYPE => 'varchar(64)', NOTNULL => '1'},
            # subtasks
            parent_bug_id		=> {TYPE => 'INT3'},
            sort_key			=> {TYPE => 'INT3'},
            resolver			=> {TYPE => 'INT3'},
            # crm
            crm					=> {TYPE => 'varchar(255)'},
            # last changed fields
            last_resolved		=> {TYPE => 'DATETIME'},
		    last_reopened		=> {TYPE => 'DATETIME'},
        ...
        INDEXES => [
            ...
            bugs_entity_idx   	      => ['entity'],
            bugs_parent_bug_id_idx    => ['parent_bug_id'],
            bugs_resolver_idx         => ['resolver'],
        ],
    },
    ...
    fielddefs => {
        FIELDS => [
            ...
            obsolete    => {TYPE => 'BOOLEAN', NOTNULL => 1,
                            DEFAULT => 'FALSE'},
            system    	=> {TYPE => 'BOOLEAN', NOTNULL => 1,
                            DEFAULT => 'FALSE'},
            system_table => {TYPE => 'BOOLEAN', NOTNULL => 1,
                            DEFAULT => 'FALSE'},
        ...
        FIELDS => [
            ...
            product_id =>  {TYPE => 'INT2', NOTNULL => 1,
                            REFERENCES => {TABLE  => 'products',
                                           COLUMN => 'id',
                                           DELETE => 'CASCADE'}},
            status	   =>  {TYPE => 'INT2', NOTNULL => 1, DEFAULT => 1},
            sortkey    => {TYPE => 'INT2', NOTNULL => 1,
                           DEFAULT => 0},
        ],
        INDEXES => [
    ...
    milestones => {
        FIELDS => [
            ...
            sortkey    => {TYPE => 'INT2', NOTNULL => 1,
                           DEFAULT => 0},
            invisible  => {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE'},
        ],
        INDEXES => [
        ...
        FIELDS => [
            ...
            userid       => {TYPE => 'INT3', NOTNULL => 1,
                             REFERENCES => {TABLE  => 'profiles',
                                            ...
                                            DELETE => 'CASCADE'}},
            name         => {TYPE => 'varchar(64)', NOTNULL => 1},
            report       => {TYPE => 'LONGTEXT', NOTNULL => 1},
        ],
        INDEXES => [
            namedreports_userid_idx => {FIELDS => [qw(userid name)],
                                        TYPE => 'UNIQUE'},
        ],
    },

    namedreports_link_in_footer => {
        FIELDS => [
            namedreport_id => {TYPE => 'INT3', NOTNULL => 1,
                              REFERENCES => {TABLE  => 'namedreports',
                                             COLUMN => 'id',
                                             DELETE => 'CASCADE'}},
            user_id       => {TYPE => 'INT3', NOTNULL => 1,
                              REFERENCES => {TABLE  => 'profiles',
        ...
        INDEXES => [
            namedreports_link_in_footer_id_idx => {FIELDS => [qw(namedreport_id user_id)],
                                                   TYPE => 'UNIQUE'},
            namedreports_link_in_footer_userid_idx => ['user_id'],
        ],
    },
        ...
        FIELDS => [
            userid       => {TYPE => 'INT3', NOTNULL => 1,
                             REFERENCES => {TABLE  => 'profiles',
                                            COLUMN => 'userid',
                                            DELETE => 'CASCADE'}},
            left_top_type => {TYPE => 'INT3'},
            left_top	  => {TYPE => 'varchar(64)'},
            left_bottom_type => {TYPE => 'INT3'},
            left_bottom	  => {TYPE => 'varchar(64)'},
            right_top_type => {TYPE => 'INT3'},
            right_top	  => {TYPE => 'varchar(64)'},
            right_bottom_type => {TYPE => 'INT3'},
            right_bottom	  => {TYPE => 'varchar(64)'},
        ],
    },
    ...
    namedreport_group_map => {
        ...
        INDEXES => [
            namedreport_group_map_namedreport_id_idx   =>
                {FIELDS => [qw(namedreport_id)], TYPE => 'UNIQUE'},
            namedreport_group_map_group_id_idx => ['group_id'],
        ],
    },
    ...
    products => {
        FIELDS => [
            ...
            viewvc_root       => {TYPE => 'MEDIUMTEXT'},
            default_entity	  => {TYPE => 'varchar(30)' }
        ],
        INDEXES => [
	...
	# entities table
	 entity => {
        FIELDS => [
        	@{ dclone(FIELD_TABLE_SCHEMA->{FIELDS}) },
            default_entity 		=> {TYPE => 'INT2', NOTNULL => 1},
   	    	subtask_allowed 	=> {TYPE => 'BOOLEAN', NOTNULL => 1},
   	    	is_subtask			=> {TYPE => 'BOOLEAN', NOTNULL => 1},
	   	    subtask_only		=> {TYPE => 'BOOLEAN', NOTNULL => 1},
	   	    subtask_valid		=> {TYPE => 'BOOLEAN', NOTNULL => 1},
	   	    subtasks_list		=> {TYPE => 'MEDIUMTEXT'},
        ],
        INDEXES => [
            entity_value_idx  => {FIELDS => ['value'], TYPE => 'UNIQUE'},
            entity_sortkey_idx => ['sortkey', 'value'],
            entity_visibility_value_id_idx => ['visibility_value_id'],
        ],
    },
    
    # browse configuration table - will be a base for the future configurable browse page
    browsedef => {
		FIELDS => [
			name			=> {TYPE => 'varchar(64)', NOTNULL => 1},
			type			=> {TYPE =>'INT2', NOTNULL => 1},
			bugs_field_name => {TYPE => 'varchar(64)', NOTNULL => 1},
			table_name		=> {TYPE => 'varchar(64)', NOTNULL => 1},
			field_name		=> {TYPE => 'varchar(64)'},
			key_field_name	=> {TYPE => 'varchar(64)'},
			search_field_name	=> {TYPE => 'varchar(64)', NOTNULL => 1},
			window			=> {TYPE =>'INT2', NOTNULL => 1},
		],
	},
