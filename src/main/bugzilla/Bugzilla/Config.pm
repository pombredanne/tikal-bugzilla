...
sub update_params {
    ...
    # update Tikal old params
    if (exists $param->{'use_subtasks'}){
    	$param->{'usesubtasks'} = $param->{'use_subtasks'};
    	delete $param->{'use_subtasks'};
    	
    }
    if (exists $param->{'show_subtasks_list'}){
    	$param->{'show_subtaskslist'} = $param->{'show_subtasks_list'};
    	delete $param->{'show_subtasks_list'};
    	
    }
    if (exists $param->{'mand_sub_sum_prefix'}){
    	$param->{'mand_sub_sumprefix'} = $param->{'mand_sub_sum_prefix'};
    	delete $param->{'mand_sub_sum_prefix'};
    	
    }
    if (exists $param->{'viewvc_url'}){
    	$param->{'viewvcurl'} = $param->{'viewvc_url'};
    	delete $param->{'viewvc_url'};
    	
    }
    if (exists $param->{'crmIdUrl'}){
    	$param->{'crm_base_url'} = $param->{'crmIdUrl'};
    	delete $param->{'crmIdUrl'};
    	
    }
    if (exists $param->{'crm_id_name'}){
    	$param->{'crm_id_field_name'} = $param->{'crm_id_name'};
    	delete $param->{'crm_id_name'};
