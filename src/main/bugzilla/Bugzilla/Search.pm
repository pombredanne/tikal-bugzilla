...
sub COLUMNS {
    ...
    my %columns = (
        ...
        resolver_realname 	 => { title => 'Resolver' 	},
    );
    ...
    my @email_fields = qw(assigned_to reporter qa_contact resolver);
...
sub init {
    ...
    foreach my $field (@select_fields) {
        ...
        next if $name eq 'component'; # components don't have sortkeys.
        next if $name eq 'version'; # versions don't have sortkeys.
    ...
    if (grep(/^resolver/, @fields)) {
        push @supptables, "LEFT JOIN profiles AS map_resolver " .
                          "ON bugs.resolver = map_resolver.userid";
    }   
    ...
    my @legal_fields = ("product", "version", "assigned_to", "reporter", "resolver", 
    ...
    if ($params->param('bug_groups')) {
        push(@specialchart, ["assigned_to", "anyexact",
                                 join(',', $params->param('bug_groups'))]);
    }
    ...
    foreach my $id ("1", "2") {
        ...
        my @clist;
        foreach my $field ("assigned_to", "reporter", "cc", "qa_contact", "resolver") {
    ...
    my @text_fields =  map { $_->name } (grep {$_->type == FIELD_TYPE_FREETEXT || $_->type == FIELD_TYPE_TEXTAREA || $_->type == FIELD_TYPE_DATETIME}
                      Bugzilla->active_custom_fields);

    foreach my $f ("short_desc", "longdesc", "bug_file_loc",
                   "status_whiteboard", "crm", @text_fields) {
    ...
    my @funcdefs = (
        "^(?:assigned_to|reporter|qa_contact|resolver),(?:notequals|equals|anyexact),%group\\.([^%]+)%" => \&_contact_exact_group,
        "^(?:assigned_to|reporter|qa_contact|resolver),(?:equals|anyexact),(%\\w+%)" => \&_contact_exact,
        "^(?:assigned_to|reporter|qa_contact|resolver),(?:notequals),(%\\w+%)" => \&_contact_notequals,
        "^(assigned_to|reporter),(?!changed)" => \&_assigned_to_reporter_nonchanged,
        "^qa_contact,(?!changed)" => \&_qa_contact_nonchanged,
        "^resolver,(?!changed)" => \&_resolver_nonchanged,
...
sub _multiselect_multiple {
	...
	# commented as a fix for bug #2422	 
