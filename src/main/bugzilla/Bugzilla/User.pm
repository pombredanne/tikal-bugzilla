...
sub reports {
	...
	my $report_ids = $dbh->selectcol_arrayref(
        'SELECT id FROM namedreports WHERE userid = ?', undef, $self->id);
	require Bugzilla::Search::SavedReport;
	$self->{reports} = Bugzilla::Search::SavedReport->new_from_list($report_ids);
	return $self->{reports};
}

sub reports_subscribed {
	my $self = shift;
	return $self->{reports_subscribed} if defined $self->{reports_subscribed};
	return [] unless $self->id;

	# Exclude the user's own reports.
	my @my_report_ids = map( $_->id, @{ $self->reports } );
	my $report_id_string = join( ',', @my_report_ids ) || '-1';

	# Only show subscriptions that we can still actually see. If a
	# user changes the shared group of a report, our subscription
	# will remain but we won't have access to the report anymore.
	my $subscribed_report_ids = Bugzilla->dbh->selectcol_arrayref(
		"SELECT lif.namedreport_id
           FROM namedreports_link_in_footer lif
                INNER JOIN namedreport_group_map ngm
                ON ngm.namedreport_id = lif.namedreport_id
          WHERE lif.user_id = ?
                AND lif.namedreport_id NOT IN ($report_id_string)
                AND ngm.group_id IN (" . $self->groups_as_string . ")",
          undef, $self->id);
	require Bugzilla::Search::SavedReport;
    $self->{reports_subscribed} =
        Bugzilla::Search::SavedReport->new_from_list($subscribed_report_ids);
	return $self->{reports_subscribed};
}

sub reports_available {
	my $self = shift;
	return $self->{reports_available} if defined $self->{reports_available};
	return [] unless $self->id;
	...
	my $avail_report_ids = Bugzilla->dbh->selectcol_arrayref(
		'SELECT namedreport_id FROM namedreport_group_map
          WHERE group_id IN (' . $self->groups_as_string . ")
                AND namedreport_id NOT IN ($report_id_string)");
	require Bugzilla::Search::SavedReport;
	$self->{reports_available} =
        Bugzilla::Search::SavedReport->new_from_list($avail_report_ids);
	return $self->{reports_available};
}

sub homepage {
	my $self = shift;
	return $self->{homepage} if defined $self->{homepage};
	return [] unless $self->id;

	# defaults
	my %homepage = ( left_top_type => USER_HOMEPAGE_TYPE_SYSTEM_BROWSE,
					 left_top => "",
					 left_bottom_type => USER_HOMEPAGE_TYPE_NONE,
					 left_bottom => "",
					 right_top_type => USER_HOMEPAGE_TYPE_USER_NEW_ISSUES,
					 right_top => "",
					 right_bottom_type => USER_HOMEPAGE_TYPE_USER_OPEN_ISSUES,
					 right_bottom => "" );

    my ($left_top_type,$left_top,$left_bottom_type,$left_bottom,$right_top_type,$right_top,$right_bottom_type,$right_bottom) = 
    		Bugzilla->dbh->selectrow_array (
															"SELECT left_top_type,left_top,left_bottom_type,left_bottom,right_top_type,right_top,right_bottom_type,right_bottom
															    		FROM user_homepage WHERE userid = ?", undef, $self->id );

	if ( $left_top_type ne "" ) {
		%homepage = ( left_top_type => $left_top_type,
					 left_top => $left_top,
					 left_bottom_type => $left_bottom_type,
					 left_bottom => $left_bottom,
					 right_top_type => $right_top_type,
					 right_top => $right_top,
					 right_bottom_type => $right_bottom_type,
...
sub flush_reports_cache {
	...
	delete $self->{reports};
	delete $self->{reports_subscribed};
	delete $self->{reports_available};
}
...
sub can_enter_product {
	...
	# It could have no released versions...
	elsif ( !@{ $product->released_versions } ) {
...
sub login_to_realname {
	...
	# statement only, so it's safe to simply trick_taint.
	trick_taint($login);
    my $user_realname = $dbh->selectrow_array("SELECT realname FROM profiles WHERE " .
                                        $dbh->sql_istrcmp('login_name', '?'),
                                        undef, $login);
	if ($user_realname) {
...
sub get_saved_search_query {

	my ($self, $query_name) = @_;

	require Bugzilla::CGI;
	require Bugzilla::Search;
	my $dbh = Bugzilla->dbh;

	my $buffer = $dbh->selectrow_array("SELECT query FROM namedqueries WHERE userid = ? AND name = ?",
										undef, $self->id, $query_name);
								
	my $params = new Bugzilla::CGI($buffer);
	my @selectcolumns = ('bug_id','bug_status','short_desc');
	my $order = $params->param('order') || "bug_status";
...
sub get_user_issues {
	...
	my ($self, $query_type, $query_name) = @_;
	
	my $dbh = Bugzilla->dbh;
	my $sth;
	
	if ($query_type == USER_HOMEPAGE_TYPE_USER_SAVED_SEARCH) {
		
		$sth = $dbh->prepare($self->get_saved_search_query($query_name));
		$sth->execute();
	} else {
		my $query;
		if ($query_type == USER_HOMEPAGE_TYPE_USER_NEW_ISSUES) {
			$query = "SELECT bug_id,bug_status,short_desc from bugs where assigned_to=? and bug_status='NEW' order by lastdiffed";
		} elsif ($query_type == USER_HOMEPAGE_TYPE_USER_OPEN_ISSUES) {
			$query = "SELECT bug_id,bug_status,short_desc from bugs where assigned_to=? and bug_status in ('ASSIGNED','REOPENED') order by bug_status, lastdiffed";
		}
	...
	my @issues;
	while ( my ( $bug_id, $status, $summary ) = $sth->fetchrow_array ) {
...
=head2 Saved and Shared Reports

=over

=item C<reports>

Returns an arrayref of the user's own saved reports, sorted by name. The
array contains L<Bugzilla::Search::Saved> objects.

=item C<reports_subscribed>

Returns an arrayref of shared reports that the user has subscribed to.
That is, these are shared reports that the user sees in their footer.
This array contains L<Bugzilla::Search::Saved> objects.

=item C<reports_available>

Returns an arrayref of all reports to which the user could possibly
subscribe. This includes the contents of L</reports_subscribed>.
An array of L<Bugzilla::Search::Saved> objects.
