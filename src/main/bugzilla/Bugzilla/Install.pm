...
sub SETTINGS {
    ...
    # Tikal
    skin               => { subclass => 'Skin', default => 'Tikal', is_enabled => 0 },
...
sub create_default_product {
    ...
    if (!$dbh->selectrow_array('SELECT 1 FROM products')) {
        ...
        # The default version.
        Bugzilla::Version::create(Bugzilla::Version::DEFAULT_VERSION, $product, VERSION_STATUS_RELEASED,0);
...
sub create_default_entity{
	#MIO_TODO add parameters for default entity

        my $default_entity = (Bugzilla->params->{'defaultentity'} || "Bug") ;
		my $dbh = Bugzilla->dbh;
	 	if (!$dbh->selectrow_array('SELECT 1 FROM entity')) {
			print "Creating default issue type 'Bug' \n";
			$dbh->do("INSERT INTO entity (value, sortkey, default_entity, subtask_allowed, is_subtask)
	    	                  VALUES (?,?,?,?,?) ",undef, 'Bug' , 100, 1, 1, 1) ;
	    	# update all existing issues with default entities
	    	$dbh->do("UPDATE bugs set entity='Bug'") ;
	 	}
}

sub create_default_browse_conf {

	my $dbh = Bugzilla->dbh;

	# If no records are defined in browsedef table add the default ones
	my $count_browsedef = $dbh->selectrow_array("SELECT count(*) FROM browsedef");
	if (!$count_browsedef) {
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Severity', 1, 'bug_severity', 'bugs', NULL, NULL, 'bug_severity', 1)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Target Milestone', 1, 'target_milestone', 'bugs', NULL, NULL, 'target_milestone', 2)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Status', 1, 'bug_status', 'bugs', NULL, NULL, 'bug_status', 3)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Component', 1, 'component_id', 'components', 'name', 'id', 'component', 4)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Assignee', 1, 'assigned_to', 'profiles', 'login_name', 'userid', 'emailassigned_to1=1&emailtype1=exact&email1', 5)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Severity', 2, 'bug_severity', 'bugs', NULL, NULL, 'bug_severity', 1)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Target Milestone', 2, 'target_milestone', 'bugs', NULL, NULL, 'target_milestone', 2)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Status', 2, 'bug_status', 'bugs', NULL, NULL, 'bug_status', 3)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Product', 2, 'product_id', 'products', 'name', 'id', 'product', 4)");
	    $dbh->do("INSERT INTO browsedef (name, type, bugs_field_name, table_name, field_name, key_field_name, search_field_name, window)
							VALUES ('Assignee', 2, 'assigned_to', 'profiles', 'login_name', 'userid', 'emailassigned_to1=1&emailtype1=exact&email1', 5)");
	}
}
