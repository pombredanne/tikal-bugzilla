...
sub UPDATE_COLUMNS {
    ...
    ### Tikal fields
    push(@columns,qw(entity parent_bug_id sort_key resolver crm last_resolved last_reopened last_verified last_closed));
...
# C<entity>			- issue type.
# C<parent_bug_id>	- create the issue as subtask to this parent
sub create {
    my ($class, $params) = @_;
    my $dbh = Bugzilla->dbh;
    ...
    # change crm field name for bugs table
    $params->{crm} =  delete $params->{crm_list};
    ...
    # Add CRM Ids list
    $bug->set_crm_list($params->{crm});
    $bug->update_crm_list(1);
...
sub update {
    ...
    foreach my $field (qw(qa_contact assigned_to resolver)) {
        if ($changes->{$field}) {
            my ($from, $to) = @{ $changes->{$field} };
            if($old_bug->$field ne '' && $self->$field ne '') {
    ...
    # Update CRM ids list
    if ($old_bug->{'crm_list'} ne $self->{'crm_list'}){
    	 $self->update_crm_list(0);
    }
    ...
    foreach my $field (@multi_selects) {
        ...
        if (scalar @$removed || scalar @$added) {
            ...
            # update for the value in the bugs table as well
            my @sorted_values = sort(@{$self->$name});
            my $new_value = join(' ', @sorted_values);
            $dbh->do("UPDATE bugs set $name = ? WHERE bug_id = ?",
...
sub _extract_multi_selects {
    ...
    foreach my $field (@multi_selects) {
        ...
        if (exists $params->{$name}) {
            $ms_values{$name} = $params->{$name} || [];
            my @ms_sorted_values = sort(@{$params->{$name}});
            $params->{$name} = join(' ', @ms_sorted_values); 
        }
    }
    return \%ms_values;
}
...
sub _check_bug_status {
    ...
    if (ref $invocant && $new_status->name eq 'ASSIGNED'
        ...
        # visible target milestones are defined for the product.
        && scalar(@{ $product->visible_milestones }) > 1
...
sub _check_multi_select_field {
    ...
    return [] if !$values;
    if (scalar(@$values) == 1){
    	my @values_array = split(',',$values->[0]);
    	$values = \@values_array;
    }
...
sub fields {
   ...
   (
        ...
        # Tikal fields
		qw(entity resolver crm_list last_resolved last_reopened last_verified last_closed),
		Bugzilla->params->{'usesubtasks'} ? qw(parent_bug_id sort_key) : (),
...
sub set_cc {
    ...
    foreach my $cc (@cc_list){
    	my $user = ref $cc ? $cc : Bugzilla::User->check($cc);
    	$self->_check_strict_isolation_for_user($user);
    	push (@updated_cc_list, $user) if !grep($_->id == $user->id, @updated_cc_list);
    }
    my $cc_users = $self->cc_users;
    @$cc_users = @updated_cc_list;
}
...
sub set_entity         { $_[0]->set('entity',        $_[1]); }
sub set_estimated_time { $_[0]->set('estimated_time', $_[1]); }
sub _set_everconfirmed { $_[0]->set('everconfirmed', $_[1]); }
sub set_last_resolved  { $_[0]->set('last_resolved', $_[1]); }
sub set_last_reopened  { $_[0]->set('last_reopened', $_[1]); }
sub set_last_verified  { $_[0]->set('last_verified', $_[1]); }
sub set_last_closed	   { $_[0]->set('last_closed',	 $_[1]); }
sub set_op_sys         { $_[0]->set('op_sys',        $_[1]); }
sub set_parent_bug_id  { $_[0]->set('parent_bug_id', $_[1]); }
...
sub set_product {
    ...
    if ($product_changed && Bugzilla->usage_mode == USAGE_MODE_BROWSER) {
        ...
        if (!$verified || !$component_ok || !$version_ok || !$milestone_ok) {
            ...
            $vars{components} = [map { $_->name } @{$product->visible_components}];
            $vars{milestones} = [map { $_->name } @{$product->visible_milestones}];
            $vars{versions}   = [map { $_->name } @{$product->released_versions}];
        }
...
sub set_resolver {
    my ($self, $value) = @_;
    $self->set('resolver', $value);
    delete $self->{'resolver_obj'};
}
sub set_resolution {
...
sub set_sort_key  	   { $_[0]->set('sort_key',		 $_[1]); }
sub set_status {
    ...
    if ($new_status->is_open) {
        ...
        $self->clear_resolution();
         ...
         #  Tikal additions - last changed fields
        if (($new_status->name eq "REOPENED") && ($old_status->name ne $new_status->name) ){
    		$self->set_last_reopened($params->{'changedate'} ? $params->{'changedate'} : $timestamp);
        }
        ...
        #  Tikal additions - resolver and last changed fields
        if ($old_status->name ne $new_status->name) {
			if ($new_status->name eq "RESOLVED"){
				# set resolver
				if (Bugzilla->params->{"auto_reassign"}){
					$self->set_resolver( $self->{'_old_assigned_to'} ? $self->{'_old_assigned_to'} : $self->{'assigned_to'});
				} else {
					$self->set_resolver( Bugzilla->user->id );
				}
	    		$self->set_last_resolved($params->{'changedate'} ? $params->{'changedate'} : $timestamp);
	    	} elsif ($new_status->name eq "VERIFIED"){
	    		$self->set_last_verified($params->{'changedate'} ? $params->{'changedate'} : $timestamp);
	    	} elsif ($new_status->name eq "CLOSED"){
	    		$self->set_last_closed($params->{'changedate'} ? $params->{'changedate'} : $timestamp);
	    	}        	
        }
     }
}
...
sub comments_for_status_can_change_to {
	my ($self) = @_;

	return $self->{comments_for_status_can_change_to} if exists $self->{comments_for_status_can_change_to};
	return {} if $self->{error};
	my @comments_for_status_can_change_to;
	foreach my $state (@{$self->status->can_change_to}) {
		push @comments_for_status_can_change_to, { 
			'status_name' => $state->name, 
			'comment_required' => $state->comment_required_on_change_from($self->status) ? 1 : 0 
			};
	}
	$self->{comments_for_status_can_change_to} = \@comments_for_status_can_change_to;
	return $self->{comments_for_status_can_change_to};
...
sub crm_list_array {
	my ($self) = @_;
	return $self->{'crm_list_array'} if exists $self->{'crm_list_array'};
    return [] if $self->{'error'};
	$self->{'crm_list_array'} = Bugzilla->dbh->selectcol_arrayref(
              "SELECT value FROM bug_crm WHERE bug_id = ? ORDER BY value",
              undef, $self->id);
    return $self->{'crm_list_array'};
}
...
sub keywords {
    my ($self) = @_;
    return (map { $_->name } @{$self->keyword_objects});
}
...
sub resolver {
    my ($self) = @_;
    return $self->{'resolver_obj'} if exists $self->{'resolver_obj'};
    return undef if $self->{'error'};
    
    if ($self->{'resolver'}) {
        $self->{'resolver_obj'} = new Bugzilla::User($self->{'resolver'});
    } else {
        # XXX - This is somewhat inconsistent with the assignee/reporter 
        # methods, which will return an empty User if they get a 0. 
        # However, we're keeping it this way now, for backwards-compatibility.
        $self->{'resolver_obj'} = undef;
    }
    return $self->{'resolver_obj'};
}
...
sub choices {
    ...
    # for components, versions and milestones - if issue has invisible component/version/milestone, add it to the list
    # for versions - invisible=in Archived status
   my $components = [map($_->name, @{$self->product_obj->visible_components})];
   if ($self->{'component'} && lsearch($components, $self->{'component'}) < 0) {
		push(@$components, $self->{'component'});
   }
   my $versions = [map($_->name, @{$self->product_obj->released_versions})];
   if ($self->{'version'} && lsearch($versions, $self->{'version'}) < 0) {
		push(@$versions, $self->{'version'});
   }
   my $milestones = [map($_->name, @{$self->product_obj->visible_milestones})];
   if ($self->{'target_milestone'} && lsearch($milestones, $self->{'target_milestone'}) < 0) {
		push(@$milestones, $self->{'target_milestone'});
   }
...
sub update_crm_list {
	...
	# Set up the crm list
    if (!$new) {
    	# remove all exsiting ones
    	$dbh->do('DELETE FROM bug_crm WHERE bug_id = ?', undef,$self->bug_id );
    }
    my $sth_crm = $dbh->prepare('INSERT INTO bug_crm (bug_id, value) VALUES (?, ?)');
    foreach my $crm_id (split(/[\s,]+/, $self->{'crm_list'})){
    	trick_taint($crm_id);
        $sth_crm->execute($self->bug_id, $crm_id);
    }    
}

sub get_multi_valued_field {
	my $self = shift;
	my ($field) = @_;
...
sub set_sub_common_fields{
	
	my $self = shift;
	my ($product_id,$version,$milestone) = @_;
	if (Bugzilla->params->{'usetargetmilestone'}){
    	$self->set('target_milestone', $milestone);
    }
...
sub parent_short_desc {

	my $self = shift;
	if (!defined $self->{'parent_short_desc'}) {
		 my $parent = new Bugzilla::Bug($self->{'parent_bug_id'});
		 $self->{'parent_short_desc'} = $parent->short_desc;
	}
	return $self->{'parent_short_desc'};
}

sub subtask_allowed {

	my $self = shift;
	if (!defined $self->{'subtask_allowed'}) {
		my $entity = new Bugzilla::Entity({ 'name' => $self->{'entity'}});
		$self->{'subtask_allowed'} = $entity->subtask_allowed();
	}

	return $self->{'subtask_allowed'};
}

sub is_subtask {

	my $self = shift;
	if (!defined $self->{'is_subtask'}) {
		my $entity = new Bugzilla::Entity({ 'name' => $self->{'entity'}});
		$self->{'is_subtask'} = $entity->is_subtask();
	}

	return $self->{'is_subtask'};
}
...
sub subtask_valid {
	...
	if (!defined $self->{'subtask_valid'}) {
		my $entity = new Bugzilla::Entity({ 'name' => $self->{'entity'}});
		$self->{'subtask_valid'} = $entity->subtask_valid();
	}

	return $self->{'subtask_valid'};
}

sub subtask_valid_to_resolve {

	my $self = shift;
	my $dbh = Bugzilla->dbh;

	if (!defined $self->{'subtask_valid_to_resolve'}) {
		if ($self->subtask_valid()){
			my $count = $dbh->selectrow_array(q{SELECT count(*) FROM bugs WHERE parent_bug_id = ? and bug_status in ('UNCONFIRMED','NEW','ASSIGNED','REOPENED')},
                                				undef, $self->{'bug_id'});
            if ($count > 0){
				$self->{'subtask_valid_to_resolve'} =  0;
			} else {
				$self->{'subtask_valid_to_resolve'} = 1;
			}
	...
	return $self->{'subtask_valid_to_resolve'};
...
sub has_subtasks {
	...
	if (!defined $self->{'has_subtasks'}) {
		my $count = $dbh->selectrow_array(q{SELECT count(*) FROM bugs WHERE parent_bug_id = ?},
                                				undef, $self->{'bug_id'});
		if ($count > 0){
			$self->{'has_subtasks'} = 1;
		} else {
...
sub subtasks_list {
	...
	if (!defined $self->{'subtasks_list'}) {
    	my $ids = $dbh->selectcol_arrayref(q{
            SELECT bug_id FROM bugs WHERE parent_bug_id = ? ORDER BY sort_key,bug_id}, undef, $self->bug_id);

        my @subtasks_list;
        foreach my $id (@$ids){
        	push @subtasks_list, new Bugzilla::Bug($id);
        }
...
sub max_sort_key {
	...
	if (!defined $self->{'max_sort_key'}) {
		$self->{'max_sort_key'} = $dbh->selectrow_array(q{SELECT max(sort_key) FROM bugs WHERE parent_bug_id = ?},
                                				undef, $self->{'bug_id'});
	}

	return $self->{'max_sort_key'};
}
...
sub get_parent_fields {
	...
	my %fields  = ('product' => $self->{'product'},
				   ...
				   'crm_list' => $self->crm_list,
				   'qa_contact' => $self->qa_contact ? $self->qa_contact->login : "",);
				   
	foreach my $cf (Bugzilla->active_custom_fields){
		if ($cf->type == FIELD_TYPE_MULTI_SELECT)
		{
			$fields{$cf->name} = $self->get_multi_valued_field($cf->name);
		} elsif ($cf->type == FIELD_TYPE_SINGLE_SELECT){
			$fields{$cf->name} = $self->{$cf->name} ? $self->{$cf->name} : "---";
		} else {
			$fields{$cf->name} = $self->{$cf->name};	
		}
	}
...
sub get_subtask_prefix {

	my $self = shift;
	my($sub_entity_type,$parent_bug_id,$parent_short_desc) = @_;

	my $prefix  = "[".$parent_short_desc." (".$parent_bug_id.") - ".$sub_entity_type."]";

	return $prefix;
}

sub attach_as_subtask {
	...
	my $sort_key = $parent->max_sort_key();
	if ($sort_key == ""){
		$sort_key = 1;
	} else {
		$sort_key ++;
	...
	my $short_desc = $self->short_desc;
	if (Bugzilla->params->{'mand_sub_sumprefix'}) {
		my $short_desc_prefix  = $self->get_subtask_prefix($self->entity,$parent->bug_id,$parent->short_desc());
		$short_desc = $short_desc_prefix.$self->short_desc;
	}
	
	$self->set_summary($short_desc);
	$self->set_parent_bug_id($parent->bug_id);
	$self->set_sort_key($sort_key);
	$self->add_comment("", { type       => CMT_ATTACH_TO,
                             extra_data => $parent->bug_id});
...
sub detach_subtask {
	...
	$self->add_comment("", { type       => CMT_DETACH_FROM,
                             extra_data => $parent_bug_id});
	$self->set_parent_bug_id("");
	$self->set_sort_key("");
...
sub _validate_attribute {
    ...
    my @valid_attributes = (
        ...
        qw(error groups product_id component_id
           ...
           isopened isunconfirmed crm_list_array
           flag_types num_attachment_flag_types
           show_attachment_flags any_flags_requesteeble
           relevancy bug_file_loc_cut),
...
sub AUTOLOAD {
  ...
  *$AUTOLOAD = sub {
      ...
      if ( grep($_->name eq $attr, @{$self->{_multi_selects}}) ) {
          ...
          trick_taint($attr);
          return $self->get_multi_valued_field($attr);
      }
