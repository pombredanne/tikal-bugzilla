...
sub create {
    ...
    Bugzilla::Version::create($version, $product, VERSION_STATUS_RELEASED, 0);
...
sub set_showonhomepage { $_[0]->set('showonhomepage', $_[1]); }
sub set_viewvc_root { $_[0]->set('viewvc_root', $_[1]); }
...
sub visible_components {
    my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{visible_components}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM components
            WHERE product_id = ? and invisible !=1
            ORDER BY name}, undef, $self->id);

        require Bugzilla::Component;
        $self->{visible_components} = Bugzilla::Component->new_from_list($ids);
    }
...
sub versions {
    ...
    if (!defined $self->{versions}) {
        my $ids = $dbh->selectcol_arrayref(q{
            ...
            WHERE product_id = ?
        	ORDER BY sortkey, value}, undef, $self->id);
...
sub unreleased_versions {
    my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{unreleased_versions}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM versions
            WHERE product_id = ? and status = ?
        	ORDER BY sortkey, value}, undef, $self->id, VERSION_STATUS_ARCHIVED);

        $self->{unreleased_versions} = Bugzilla::Version->new_from_list($ids);
    }
    return $self->{unreleased_versions};
}

sub released_versions {
    my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{released_versions}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM versions
            WHERE product_id = ? AND (status = ? OR status = ?)
        	ORDER BY sortkey, value}, undef, $self->id, VERSION_STATUS_RELEASED, VERSION_STATUS_FINAL);

        $self->{released_versions} = Bugzilla::Version->new_from_list($ids);
    }
    return $self->{released_versions};
}
...
sub archived_versions {
    ...
    if (!defined $self->{archived_versions}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM versions
            WHERE product_id = ? and status = ?
        	ORDER BY sortkey, value}, undef, $self->id, VERSION_STATUS_ARCHIVED);

        $self->{archived_versions} = Bugzilla::Version->new_from_list($ids);
    }
    return $self->{archived_versions};
}

sub active_for_custom_versions {
	my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{active_for_custom_versions}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM versions
            WHERE product_id = ? and (status = ? or status = ?)
        	ORDER BY sortkey, value}, undef, $self->id,VERSION_STATUS_UNRELEASED,VERSION_STATUS_RELEASED);

        $self->{active_for_custom_versions} = Bugzilla::Version->new_from_list($ids);
    }
    return $self->{active_for_custom_versions};
}

sub active_for_custom_versions_names {
	my $self = shift;
	
	return map {$_->name} @{$self->{active_for_custom_versions}};
}
...
sub visible_milestones {
    my $self = shift;
    my $dbh = Bugzilla->dbh;

    if (!defined $self->{visible_milestones}) {
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM milestones
             WHERE product_id = ? and invisible != 1
        	ORDER BY sortkey, value}, undef, $self->id);
 
        $self->{visible_milestones} = Bugzilla::Milestone->new_from_list($ids);
    }
    return $self->{visible_milestones};
}
...
sub showonhomepage 	  { return $_[0]->{'showonhomepage'}; 	 }
sub viewvc_root		  { return $_[0]->{'viewvc_root'};		 }
...
=head1 SYNOPSIS
    ...
    my @visible_components  = $product->visible_components();
    my $groups_controls = $product->group_controls();
    my @milestones      = $product->milestones();
    my @versions        = $product->versions();
    my @visible_milestones = $product->visible_milestones();
    my @released_versions = $product->released_versions();
    ...
    my $showonhomepage   = $product->showonhomepage;
    my $viewvc_root		 = $product->viewvc_root;
    my $default_entity	 = $product->default_entity;
...
=item C<visible_components>

 Description: Returns an array of visible component objects belonging to
              the product.
...
=item C<released_versions>

 Description: Returns all released versions for that product (in status RELEASED or FINAL).

 Params:      none.

 Returns:     An array of Bugzilla::Version objects.
 
=item C<unreleased_versions>

 Description: Returns all unreleased versions for that product.

 Params:      none.

 Returns:     An array of Bugzilla::Version objects.
 
=item C<archived_versions>

 Description: Returns all archived versions for that product.

 Params:      none.

 Returns:     An array of Bugzilla::Version objects.
 
=item C<active_for_custom_versions>

 Description: Returns all released and unreleased versions for that product.
...
=item C<visible_milestones>

 Description: Returns all visible milestones for that product.
