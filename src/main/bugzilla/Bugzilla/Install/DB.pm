...
sub update_fielddefs_definition {
	...
	$dbh->bz_add_column('fielddefs', 'system', {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE'});
	$dbh->bz_add_column('fielddefs', 'system_table', {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE'});
	$dbh->bz_add_column('fielddefs', 'mandatory', {TYPE => 'INT2'});
	$dbh->bz_add_column('fielddefs', 'default_value', {TYPE => 'varchar(255)'});
...
sub update_table_definitions {
	...
	# Tikal additions
	# entity
	$dbh->bz_add_column('bugs', 'entity', {TYPE =>  'varchar(64)'});
	Bugzilla::Field->create({ name => 'entity', description => 'entity' }) unless new Bugzilla::Field({ name => 'entity' });
	$dbh->bz_rename_column('entity', 'name', 'value');
	$dbh->bz_rename_column('entity', 'entityid', 'id');
	$dbh->bz_add_column('entity', 'sortkey', {TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0});
	$dbh->bz_add_column('entity', 'isactive', {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'TRUE'});
	$dbh->bz_add_column('entity', 'visibility_value_id', {TYPE => 'INT2'});
	$dbh->bz_add_index('entity', 'entity_value_idx', {FIELDS => ['value'], TYPE => 'UNIQUE'});
	$dbh->bz_add_index('entity', 'entity_sortkey_idx', ['sortkey', 'value']);
	$dbh->bz_add_index('entity', 'entity_visibility_value_id_idx', ['visibility_value_id']);
	$dbh->bz_add_index('bugs', 'bugs_entity_idx', ['entity']);
	
	# subtasks
	if (!$dbh->bz_column_info('bugs', 'parent_bug_id')){
		$dbh->bz_add_column('bugs', 'parent_bug_id', {TYPE =>  'INT3'});
		$dbh->bz_add_column('bugs', 'sort_key', {TYPE =>  'MEDIUMINT(9)'});
		$dbh->bz_add_index('bugs', 'bugs_parent_bug_id_idx', ['parent_bug_id']);
	}
	Bugzilla::Field->create({ name => 'parent_bug_id', description => 'parent_bug_id' }) unless new Bugzilla::Field({ name => 'parent_bug_id' });

	# auto-reassign
	if (!$dbh->bz_column_info('bugs', 'resolver')){
		$dbh->bz_add_column('bugs', 'resolver', {TYPE => 'INT3'});
		$dbh->bz_add_index('bugs', 'bugs_resolver_idx', ['resolver']);
	}
	Bugzilla::Field->create({ name => 'resolver', description => 'resolver' }) unless new Bugzilla::Field({ name => 'resolver' });
	_update_resolver_for_existing_issues();
	
	# showonhomepage for products
	if ($dbh->bz_column_info('products', 'showonhomepage')){
		$dbh->bz_alter_column('products', 'showonhomepage', {TYPE => 'BOOLEAN' });
	} else {
		$dbh->bz_add_column('products', 'showonhomepage', {TYPE => 'BOOLEAN' });
		_update_showonhomepage_for_existing_products();
	}
	
	# viewvc_root and default entity for products
	$dbh->bz_add_column('products', 'viewvc_root', {TYPE => 'MEDIUMTEXT' });
	$dbh->bz_add_column('products', 'default_entity', {TYPE => 'varchar(30)' });
	
	# update namedreports table
    $dbh->bz_add_column('namedreports', 'id',{TYPE => 'MEDIUMSERIAL', NOTNULL => 1, PRIMARYKEY => 1});
    _move_namedreports_linkinfooter_to_its_own_table();
	
	# crm 
	if ($dbh->bz_column_info('bugs', 'crm_id')){
		$dbh->bz_rename_column('bugs', 'crm_id', 'crm');
		$dbh->bz_alter_column('bugs', 'crm', {TYPE => 'varchar(255)'});
		# move values to bug_crm field and drop the old table
		my $bug_crm = $dbh->selectall_arrayref("SELECT crm_value,bug_id FROM crm",undef);
		foreach my $bc (@$bug_crm){
			my ($crm_value,$bug_id) = @$bc;
			$dbh->do("INSERT INTO bug_crm (value,bug_id) VALUES (?,?)",undef,$crm_value,$bug_id);
			$dbh->bz_drop_table('crm');
		}
	} else {
		$dbh->bz_add_column('bugs', 'crm', {TYPE => 'varchar(255)'});
	}
	Bugzilla::Field->create({ name => 'crm', description => Bugzilla->params->{'crm_id_field_name'}}) unless new Bugzilla::Field({ name => 'crm' });
	
	# invisible indicators
	$dbh->bz_add_column('components', 'invisible', {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE'});
	$dbh->bz_add_column('milestones', 'invisible', {TYPE => 'BOOLEAN', NOTNULL => 1, DEFAULT => 'FALSE'});
	
	# sort for versions
	$dbh->bz_add_column('versions', 'sortkey', {TYPE => 'INT2', NOTNULL => 1, DEFAULT => 0});
	
	# last changged fields
	if ($dbh->bz_column_info('bugs', 'last_resolved')){
		$dbh->bz_alter_column('bugs', 'last_resolved', {TYPE => 'DATETIME'});
		$dbh->bz_alter_column('bugs', 'last_reopened', {TYPE => 'DATETIME'});
		$dbh->bz_alter_column('bugs', 'last_verified', {TYPE => 'DATETIME'});
		$dbh->bz_alter_column('bugs', 'last_closed',   {TYPE => 'DATETIME'});
	} else {
		$dbh->bz_add_column('bugs', 'last_resolved', {TYPE => 'DATETIME'});
		$dbh->bz_add_column('bugs', 'last_reopened', {TYPE => 'DATETIME'});
		$dbh->bz_add_column('bugs', 'last_verified', {TYPE => 'DATETIME'});
		$dbh->bz_add_column('bugs', 'last_closed',	 {TYPE => 'DATETIME'});
	}
	
	# whine_events
	$dbh->bz_add_column('whine_events', 'query_fields', {TYPE => 'MEDIUMTEXT'});

	# remove old features
	$dbh->do("DELETE FROM entity WHERE value='Release'");
	$dbh->bz_drop_column('products', 'autoalias_prefix');
	$dbh->bz_drop_column('components', 'category');
	$dbh->bz_drop_table('componentsctg');
	$dbh->bz_drop_column('keyworddefs', 'category');
	$dbh->bz_drop_table('keywordsctg');
	$dbh->bz_drop_column('versions', 'classification_id');
	$dbh->bz_drop_column('milestones', 'classification_id');
	$dbh->bz_drop_column('milestones', 'win_build_home');
	$dbh->bz_drop_column('milestones', 'linux_build_home');
	$dbh->bz_drop_column('milestones', 'solaris_build_home');
	$dbh->bz_drop_column('entity', 'autosubtask_parent');
	$dbh->bz_drop_column('entity', 'autosubtask_child');
	$dbh->bz_drop_column('entity', 'autosubtask_def_comp_id');
	$dbh->bz_drop_column('entity', 'release_allowed');
	$dbh->bz_drop_column('entity', 'release_entity');
	$dbh->bz_drop_column('entity', 'autoalias_type');
	$dbh->bz_drop_column('entity', 'autoalias_prefix');
	$dbh->bz_drop_column('bugs', 'comment_id');
	$dbh->bz_drop_index('bugs', 'bugs_parent_release_bug_id_idx');
	$dbh->bz_drop_table('custom_enum');
	# remove old user settings
	$dbh->do("DELETE FROM profile_setting WHERE setting_name in ('subtasks_display','comment_display')");
	$dbh->do("DELETE FROM setting_value WHERE name in ('subtasks_display','comment_display')");
	$dbh->do("DELETE FROM setting WHERE name in ('subtasks_display','comment_display')");
	
	# fix DB - remove "dead" records
	$dbh->do("delete from bugs_activity where fieldid not in (select id from fielddefs)");
	
	# version status
	$dbh->bz_add_column('versions', 'status', {TYPE => 'INT2', NOTNULL => 1, DEFAULT => 1});
	_update_status_for_existing_versions();
	$dbh->bz_drop_column('versions', 'invisible');
	
	# set the Tikal skin
	$dbh->do("UPDATE setting SET default_value='Tikal', is_enabled=0 WHERE name='skin'");
	$dbh->do("DELETE FROM profile_setting WHERE setting_name='skin'");
	
	# make cf_reopen_resolution field to be system if defined
	$dbh->do("UPDATE fielddefs SET system=1 WHERE name='cf_reopen_resolution'");
...
sub _move_namedreports_linkinfooter_to_its_own_table {
    my $dbh = Bugzilla->dbh;
    if ($dbh->bz_column_info("namedreports", "linkinfooter")) {
        # Move link-in-footer information into a table of its own.
        my $sth_read = $dbh->prepare('SELECT id, userid
                                        FROM namedreports
                                       WHERE linkinfooter = 1');
        my $sth_write = $dbh->prepare('INSERT INTO namedreports_link_in_footer
                                       (namedreport_id, user_id) VALUES (?, ?)');
        $sth_read->execute();
        while (my ($id, $userid) = $sth_read->fetchrow_array()) {
            $sth_write->execute($id, $userid);
        }
        $dbh->bz_drop_column("namedreports", "linkinfooter");
    }
}
...
# Tikal subroutines
sub _update_resolver_for_existing_issues {
	 my $dbh = Bugzilla->dbh;
	 my $issues_to_fix = $dbh->selectall_arrayref(
        "SELECT bug_id, assigned_to FROM bugs WHERE bug_status = 'RESOLVED' AND resolution = 'FIXED' AND resolver is NULL", undef);
    foreach my $issue (@$issues_to_fix) {
    	my ($bug_id, $user_id) = @$issue;
 		$dbh->do('UPDATE bugs SET resolver = ? WHERE bug_id = ? ',undef, $user_id, $bug_id);
    }
}
    
sub _update_showonhomepage_for_existing_products() {
	my $dbh = Bugzilla->dbh;
	$dbh->do('UPDATE products SET showonhomepage = 1 WHERE showonhomepage is NULL',undef);
}

sub _update_status_for_existing_versions(){
	my $dbh = Bugzilla->dbh;
	if ($dbh->bz_column_info('versions', 'invisible')){
		$dbh->do('UPDATE versions SET status = 2 WHERE invisible=1',undef);
	}
	$dbh->do('UPDATE versions SET status = 1 WHERE status is NULL',undef);
}
