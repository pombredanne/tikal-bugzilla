...
use constant VALIDATORS => {
    ...
    system_table => \&_check_system_table,
...
use constant SQL_DEFINITIONS => {
    ...
    FIELD_TYPE_SINGLE_SELECT, { TYPE => 'varchar(64)', NOTNULL => 1,
                                DEFAULT => "'---'" },
    FIELD_TYPE_MULTI_SELECT,  { TYPE => 'varchar(255)'},
...
use constant DEFAULT_FIELDS => (
    ...
    {name => 'qa_contact',   desc => 'QAContact',  in_new_bugmail => 1,
     buglist => 1},
    {name => 'resolver',   	 desc => 'Resolver',   in_new_bugmail => 1,
     buglist => 1},
    ...
    # Tikal fields
    {name => 'entity',			      desc => 'Issue Type', in_new_bugmail => 1,
     type => FIELD_TYPE_SINGLE_SELECT, buglist => 1},
    {name => 'parent_bug_id',		  desc => 'Parent Id', , buglist => 1, in_new_bugmail => 1},
    {name => 'sort_key',		      desc => 'Subtask sort key'},
    {name => 'crm',					  desc => 'CRM Ids', buglist => 1, in_new_bugmail => 1},
    {name => 'resolver',   	 		  desc => 'Resolver', buglist => 1},
    {name => 'last_resolved',		  desc => 'Resolved Date', buglist => 1},
    {name => 'last_reopened',		  desc => 'Reopened Date', buglist => 1},
    {name => 'last_verified',		  desc => 'Verified Date', buglist => 1},
    {name => 'last_closed',			  desc => 'Closed Date', buglist => 1},
);
...
# Override match to add is_select and is_text.
sub match {
    my $self = shift;
    my ($params) = @_;
    ...
    if (delete $params->{is_text}) {
        $params->{type} = [FIELD_TYPE_FREETEXT, FIELD_TYPE_DATETIME, FIELD_TYPE_TEXTAREA, FIELD_TYPE_BUG_ID];
    }
...
sub _check_system { return $_[1] ? 1 : 0; }

sub _check_system_table { return $_[1] ? 1 : 0; }
...
sub _check_control_value {
    ...
    if ($field->name eq "component"){
    	$value_obj = Bugzilla::Component->new($value_id);
    } elsif ($field->name eq "version"){
    	$value_obj = Bugzilla::Version->new($value_id);
    } else {
    	$value_obj = Bugzilla::Field::Choice->type($field)
                    ->check({ id => $value_id });
    }
    return $value_obj->id;
}
...
True if this is a C<FIELD_TYPE_FREETEXT>, <FIELD_TYPE_DATETIME>, <FIELD_TYPE_TEXTAREA> or <FIELD_TYPE_BUG_ID>
field. 
...
sub is_text { 
    return ($_[0]->type == FIELD_TYPE_FREETEXT 
            || $_[0]->type == FIELD_TYPE_DATETIME
            || $_[0]->type == FIELD_TYPE_TEXTAREA
            || $_[0]->type == FIELD_TYPE_BUG_ID ) ? 1 : 0 
}
...
sub legal_values {
    ...
    if (!defined $self->{'legal_values'}) {
    	my @values;
    	if ($self->name eq "entity"){
    		require Bugzilla::Entity;
    		@values = Bugzilla::Entity->get_legal_values($issue_types);
    	} else {
    		require Bugzilla::Field::Choice;
        	@values = Bugzilla::Field::Choice->type($self)->get_all();
    	}
...
=item C<active_legal_values>

Valid values for this field, as an array of L<Bugzilla::Field::Choice>
objects, while isactive is set to 1

=back

=cut
sub active_legal_values {
    my $self = shift;
    
    if (!defined $self->{'active_legal_values'}) {
    	
    	if ($self->custom){
    		my @values;
    		require Bugzilla::Field::Choice;
        	@values = grep {$_->{'isactive'} == 1} Bugzilla::Field::Choice->type($self)->get_all();
			my $self_value = shift;
			my $values_names = [map($_->name, @values)];
			if ($self_value && lsearch($values_names, $self_value) < 0) {
				my $fvalue = Bugzilla::Field::Choice->type($self)->new({"name" => $self_value});
				push(@values, $fvalue);
			}
        	$self->{'active_legal_values'} = \@values;
    	} else {
    		$self->{'active_legal_values'} = $self->{'legal_values'};
    	}
    }
    return $self->{'active_legal_values'};
}

# ugly code - replace when https://bugzilla.mozilla.org/show_bug.cgi?id=487508 is fixed
sub legal_values_by_product {
    my $self = shift;
    my $product_id= shift;
    
    my @values;
    require Bugzilla::Product;
	my $product = Bugzilla::Product->new($product_id);
	if ($self->name eq "component"){
		my $self_comp = shift;
		@values = @{$product->visible_components};
		my $values_names = [map($_->name, @values)];
		if ($self_comp && lsearch($values_names, $self_comp) < 0) {
			require Bugzilla::Component;
			my $component = Bugzilla::Component->new({"name" => $self_comp, "product" => $product});
			push(@values, $component);
	   }
	} elsif($self->name eq "version") {
		my $self_vers = shift;
		@values = @{$product->released_versions};
		my $values_names = [map($_->name, @values)];
		if ($self_vers && lsearch($values_names, $self_vers) < 0) {
			require Bugzilla::Version;
			my $version = Bugzilla::Version->new({"name" => $self_vers, "product" => $product});
			push(@values, $version);
	   }
	}
    return @values;
}
sub legal_values_with_product {
    my $self = shift;
    my $product_id= shift;
    
    my @values;
	if ($self->name eq "component" || $self->name eq "version"){
    	my @products = Bugzilla::Field::Choice->type(Bugzilla::Field->new({'name' => 'product'}))->get_all();
		foreach my $product (@products) {
			if ($self->name eq "component") {
				push (@values, { "id" => $_->id, "name" => $product->name.": ".$_->name}) foreach (@{$product->components});
			} else {
				push (@values, { "id" => $_->id, "name" => $product->name.": ".$_->name}) foreach (@{$product->versions});
			}
		}
	}
    return @values;
...
a boolean specifying whether or not the field is system custom field;

=back

=cut

sub system { return $_[0]->{system} }

=over

=item C<system_table>

A system table to use for values instead of default one (at the moment 'versions' table only supported)
=back

=cut

sub system_table { return $_[0]->{system_table} }

=over

=item C<mandatory>

Specifes whether the custom field is mandatory. Values:
0 - not mandatory
1 - always mandatory
2 - mandatory for new issue only
3 - mandatory for existong issue (not new)
4 - mandatory when changing status to RESOLVED FIXED
5 - mandatory when changing status to REOPENED

=back

=cut

sub mandatory { return $_[0]->{mandatory} }

=over

=item C<default_value>

A default value for custom field
=back

=cut

sub default_value { return $_[0]->{default_value} }
...
=item C<set_system_table>

=item C<set_mandatory>

=item C<set_default_value>
...
sub set_system	       { $_[0]->set('system',	   $_[1]); }
sub set_system_table   { $_[0]->set('system_table',	   $_[1]); }
sub set_sortkey        { $_[0]->set('sortkey',     $_[1]); }
sub set_in_new_bugmail { $_[0]->set('mailhead',    $_[1]); }
sub set_mandatory	   { $_[0]->set('mandatory',   $_[1]); }
...
C<system> - boolean - Whether this field is system custom field. Defaults to 0.

C<system_table> - boolean - Whether this system custom field is taking values from 'versions' table. Defaults to 0.
...
sub create {
    ...
    if ($field->custom) {
        ...
        if ($type == FIELD_TYPE_SINGLE_SELECT && !$field->system_table) {
...
sub check_field {
        ...
        || lsearch($legalsRef, $value) < 0)
    {
        ...
        my $field = new Bugzilla::Field({ name => $name });
        return 0 if ($field->system_table); # This field's legal values are taken from system table, not checked
