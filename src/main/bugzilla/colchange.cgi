...
if (Bugzilla->params->{"usesubtasks"}) {
	@masterlist = ("parent_bug_id", "opendate", "changeddate", "bug_severity", "priority",
                  "rep_platform", "assigned_to", "assigned_to_realname",
                  "reporter", "reporter_realname", "bug_status",
                  "resolution", "entity");
} else {
	@masterlist = ("opendate", "changeddate", "bug_severity", "priority",
                  "rep_platform", "assigned_to", "assigned_to_realname",
                  "reporter", "reporter_realname", "bug_status",
                  "resolution", "entity");
}
...
push(@masterlist, ("product", "component", "version", "op_sys","resolver","resolver_realname"));

if (Bugzilla->params->{"use_crm_id"}) {
    push (@masterlist, "crm");
}
...
if (Bugzilla->params->{"use_last_changed_fields"}) {
    push(@masterlist, "last_resolved");
    push(@masterlist, "last_reopened");
    push(@masterlist, "last_verified");
    push(@masterlist, "last_closed");
}
...
my @custom_fields = grep {$_->type != FIELD_TYPE_TEXTAREA}
