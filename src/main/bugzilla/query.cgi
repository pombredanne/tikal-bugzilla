...
sub PrefillForm {
    ...
    my @list = ("bug_status", "resolution", "assigned_to",
                      ...
                      "newsubcategory", "public", "frequency", "entity", 
                      "crm","crm_type", "bug_groups");
  
    push(@list, map { $_->name } Bugzilla->active_custom_fields);

    foreach my $name (@list) {
        $default{$name} = [];
    }

    my @cf_text_names = map {$_->name} (grep {$_->type == FIELD_TYPE_FREETEXT || $_->type == FIELD_TYPE_TEXTAREA || $_->type == FIELD_TYPE_DATETIME}
                      Bugzilla->active_custom_fields);
    foreach my $cf_text_name (@cf_text_names){
    	$default{$cf_text_name."_type"} = [];
    }
...
my $groups = $dbh->selectall_arrayref("SELECT id,name FROM groups where isbuggroup = '1'", undef);
	foreach my $group (@$groups) {
		my ($group_id,$name) = @$group;
		$bug_groups{$group_id}=$name;
	}

my $user_ids_as_list = "";
foreach my $id (keys %bug_groups) {
	
	my $login_names = $dbh->selectcol_arrayref("SELECT login_name FROM user_group_map,profiles WHERE user_group_map.group_id =? AND user_group_map.user_id = profiles.userid",undef,$id);
	foreach my $login_name (@$login_names){
		if(!$user_per_group{$login_name}){
			$user_per_group{$login_name}=$login_name;
			$user_ids_as_list = $user_ids_as_list ."$login_name ,";
		}
	}
	chop $user_ids_as_list if ($user_ids_as_list ne "");

	push @user_values,$id;
	$bug_group_hash{$id}=$user_ids_as_list;
	$bug_group_names{$id}=$bug_groups{$id};
	$user_ids_as_list = "";
	%user_per_group = ();
}

$vars->{'bug_groups_hash'} = \%bug_group_hash;
$vars->{'bug_groups_value'} = \@user_values;
$vars->{'bug_groups_names'} = \%bug_group_names;
