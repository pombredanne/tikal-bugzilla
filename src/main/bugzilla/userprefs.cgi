...
sub SaveSavedReports {
    ...
    my $sth_insert_nl = $dbh->prepare('INSERT INTO namedreports_link_in_footer
                                       (namedreport_id, user_id)
                                       VALUES (?, ?)');
    my $sth_delete_nl = $dbh->prepare('DELETE FROM namedreports_link_in_footer
                                             WHERE namedreport_id = ?
                                               AND user_id = ?');
    my $sth_insert_ngm = $dbh->prepare('INSERT INTO namedreport_group_map
                                        (namedreport_id, group_id)
                                        VALUES (?, ?)');
    my $sth_update_ngm = $dbh->prepare('UPDATE namedreport_group_map
                                           SET group_id = ?
                                         WHERE namedreport_id = ?');
    my $sth_delete_ngm = $dbh->prepare('DELETE FROM namedreport_group_map
                                              WHERE namedreport_id = ?');

    # Update namedreports_link_in_footer for this user.
    foreach my $q (@{$user->reports}, @{$user->reports_available}) {
        if (defined $cgi->param("link_in_footer_" . $q->id)) {
            $sth_insert_nl->execute($q->id, $user_id) if !$q->link_in_footer;
        }
        else {
    ...
    # For user's own reports, update namedreport_group_map.
    foreach my $q (@{$user->reports}) {
        my $group_id;
        ...
        if ($group_id) {
            # Don't allow the user to share reports with groups he's not
            # allowed to.
            next unless grep($_ eq $group_id, @{$user->queryshare_groups});

            # $group_id is now definitely a valid ID of a group the
            # user can share reports with, so we can trick_taint.
            detaint_natural($group_id);
            if ($q->shared_with_group) {
                $sth_update_ngm->execute($group_id, $q->id);
            }
            ...
            # If we're sharing our  with a group we can bless, we 
            # have the ability to add link to our search to the footer of
            # direct group members automatically.
            if ($user->can_bless($group_id) && $cgi->param('force_' . $q->id)) {
                my $group = new Bugzilla::Group($group_id);
            ...
            # They have unshared that .
            if ($q->shared_with_group) {
                $sth_delete_ngm->execute($q->id);
            }

            # Don't remove namedreports_link_in_footer entries for users
            # subscribing to the shared . The idea is that they will
            # probably want to be subscribers again should the sharing
            # user choose to share the  again.
        }
    }
...
sub SaveHomepage {
    ...
    my $left_top_type = $cgi->param("left_top_type") || "";
    my $left_top = $cgi->param("left_top") || "";
    my $left_bottom_type = $cgi->param("left_bottom_type") || "";
    my $left_bottom = $cgi->param("left_bottom") || "";
    my $right_top_type = $cgi->param("right_top_type") || "";
    my $right_top = $cgi->param("right_top") || "";
    my $right_bottom_type = $cgi->param("right_bottom_type") || "";
    my $right_bottom = $cgi->param("right_bottom") || "";
    trick_taint($left_top);
    trick_taint($left_top_type);
    trick_taint($left_bottom);
    trick_taint($left_bottom_type);
    trick_taint($right_top);
    trick_taint($right_top_type);
    trick_taint($right_bottom);
    trick_taint($right_bottom_type);
    
    my $userid = $dbh->selectrow_array ("SELECT userid FROM user_homepage WHERE userid = ?", undef, $user->id);
    if ($userid) {
    	$dbh->do("UPDATE user_homepage SET left_top_type = ?, left_top = ?, left_bottom_type = ?, left_bottom = ?, 
    	                                   right_top_type = ?, right_top = ?, right_bottom_type = ?, right_bottom = ?
    	 			WHERE userid = ?", undef, ($left_top_type,$left_top,$left_bottom_type,$left_bottom,$right_top_type,$right_top,$right_bottom_type,$right_bottom, $user->id)); 
    } else {
    	$dbh->do("INSERT INTO user_homepage (userid,left_top_type,left_top,left_bottom_type,left_bottom,right_top_type,right_top,right_bottom_type,right_bottom) 
    								VALUES (?,?,?,?,?,?,?,?,?)", undef, ($user->id,$left_top_type,$left_top,$left_bottom_type,$left_bottom,$right_top_type,$right_top,$right_bottom_type,$right_bottom)); 
    }
...
SWITCH: for ($current_tab_name) {
    ...
    /^saved-reports$/ && do {
        SaveSavedReports() if $cgi->param('dosave');
        DoSavedReports();
        last SWITCH;
    };   
    /^homepage$/ && do {
        SaveHomepage() if $cgi->param('dosave');
        DoHomepage();
        last SWITCH;
    };    
