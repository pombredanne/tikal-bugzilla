...
sub active_custom_system_fields {
    my $class = shift;
    if (!exists $class->request_cache->{active_custom_system_fields}) {
        $class->request_cache->{active_custom_system_fields} =
          Bugzilla::Field->match({ custom => 1, system => 1, obsolete => 0 });
    }
    return @{$class->request_cache->{active_custom_system_fields}};
}

sub get_custom_system_field {
    my $class = shift;
    my ($field_name) = @_;
    
    my $field = new Bugzilla::Field ({name => $field_name});
    return $field if ($field && !$field->obsolete);
	return undef;
}

sub get_all_active_for_custom_versions {
	my $class = shift;
    my $dbh = Bugzilla->dbh;

    if (!exists $class->request_cache->{get_all_active_for_custom_versions}) {
        
        my $ids = $dbh->selectcol_arrayref(q{
            SELECT id FROM versions
            WHERE status = ? or status = ?}, undef, VERSION_STATUS_UNRELEASED,VERSION_STATUS_RELEASED);

        $class->request_cache->{get_all_active_for_custom_versions} = Bugzilla::Version->new_from_list($ids);
    }
    return @{$class->request_cache->{get_all_active_for_custom_versions}};
}

sub get_all_active_for_custom_versions_names {
	my $class = shift;

	return map {$_->name} $class->get_all_active_for_custom_versions();
}
...
sub get_all_keywords {
    ...
    if (!exists $class->request_cache->{get_all_keywords}) {

        $class->request_cache->{get_all_keywords} =  
        		$dbh->selectcol_arrayref(q{SELECT name FROM keyworddefs ORDER BY name}, undef);
    }
    return @{$class->request_cache->{get_all_keywords}};
}
