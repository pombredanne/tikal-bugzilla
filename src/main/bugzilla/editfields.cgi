...
# Interface to add a new custom field.
    ...
    $vars->{'field'} = Bugzilla::Field->create({
        ...
        system    	=> scalar $cgi->param('system'),
        system_table => scalar $cgi->param('system_table'),
        mandatory	=> scalar $cgi->param('mandatory'),
        default_value	=> scalar $cgi->param('default_value'),
        custom      => 1,
        buglist     => 1,
    ...
    $field->set_system($cgi->param('system'));
    $field->set_mandatory($cgi->param('mandatory'));
    $field->set_default_value($cgi->param('default_value'));
