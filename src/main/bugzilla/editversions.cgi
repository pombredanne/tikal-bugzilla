...
my $status    	 = trim($cgi->param('status') || '');
my $sortkey    	 = trim($cgi->param('sortkey') || '');
...
if ($action eq 'new') {
    ...
    my $version = Bugzilla::Version::create($version_name, $product, $status, $sortkey);
    delete_token($token);
...
if ($action eq 'update') {
    ...
    my $version_old_status = trim($cgi->param('statusold') || '');
    my $version_old_sortkey = trim($cgi->param('sortkeyold') || '');
    my $version = Bugzilla::Version->check({ product => $product,
                                             name   => $version_old_name });

    $dbh->bz_start_transaction();

    $vars->{'updated'} = $version->update($version_name, $product, $status, $sortkey);

    $dbh->bz_commit_transaction();
    delete_token($token);
    
    my $message = "version_updated";
    if ($version->status ne $version_old_status){
    	$message = "version_status_updated";
    } elsif ($version->sortkey ne $version_old_sortkey){
    	$message = "version_sortkey_updated";
    }
