...
function showValueWhen(controlled_field_id, controlled_value_ids, controlled_value_names, 
                       controller_field_id, controller_value_id)
{
    ...
    var argsArray = [controlled_field_id, controlled_value_ids, controlled_value_names, controller_field, controller_value_id];
    if (controller_field.nodeName == "INPUT"){
        	$(controller_field).bind(controller_field_id + "-change", 
        		{args: argsArray},
            function(ev){
                handleValControllerChange(ev, ev.data.args);
        });
        
    } else {
		YAHOO.util.Event.addListener(controller_field, 'change',
				handleValControllerChange, argsArray);
	}
}

function handleValControllerChange(e, args) {
	var controlled_field_name = args[0];
    var controlled_field = document.getElementById(args[0]);
    var controlled_value_ids = args[1];
    var controlled_value_names = args[2];
    var controller_field = args[3];
    var controller_value_id = args[4];
    
    if (controlled_field){
    	  
	    var controllerSelected = false;
	    // different handling for controller multivalued fields with items-selector
	    if (controller_field.nodeName == "INPUT"){
	    	if (controller_field.value.indexOf(controller_value_id) != -1 ){
	    		controllerSelected = true;
	    	}
	    } else {
		    var controller_item = document.getElementById(_value_id(controller_field.id, controller_value_id));
		    if (controller_item && controller_item.selected) {
		    	controllerSelected = true;
		    }
	    }
	    
	    // different handling for controlled multivalued fields with items-selector
	    if (controlled_field.nodeName == "INPUT"){
	    	for (var i = 0; i < controlled_value_names.length; i++) {
	    		controlled_field = document.getElementById(controlled_field_name+"_"+controlled_value_names[i]);
	    		if (controlled_field){ 
		    		if (controllerSelected) {
		    			toggleValue4MultiComp(controlled_field_name+"_"+controlled_value_names[i],true);
		    		} else {
		    			toggleValue4MultiComp(controlled_field_name+"_"+controlled_value_names[i],false);
		    		}
	    		}
	    	}
	    	...
	    	for (var i = 0; i < controlled_value_ids.length; i++) {
	            ...
	            if (item.disabled && controllerSelected) {
...
// Show or hide values of the multivalued fields
function toggleValue4MultiComp(valueid, show) {
	var valueLi = document.getElementById(valueid);
	if (valueLi != null) {
		valueLi.style.display = show ? 'block' : 'none';
	}
}
...
function initHidingOptionsForIE(select_name) {
    ...
    if (aSelect != null && browserCanHideOptions(aSelect)) return;
