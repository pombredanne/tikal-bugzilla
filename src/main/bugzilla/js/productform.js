 ...
 *                    there is no such element to update.
 * @param  system_cf  Array of system custom fields that have values from versions. Can be null if
 *                    there are no such elements to update.
 *
 ...
 */
function selectClassification(classfield, product, component, version, milestone, system_cf) {
    ...
    selectProduct(product, component, version, milestone, system_cf, null);
}
 ...
 *                    has changed, and optimize for additions.
 */
function selectProduct(product, component, version, milestone, system_cf, anyval) {
    // This is to avoid handling events that occur before the form
    // itself is ready, which could happen in buggy browsers.
    if (!product)
        return;
    ...
    if (system_cf && (system_cf instanceof Array) && system_cf.length > 0) {
    	for (var i = 0; i < system_cf.length; i++) {
    		var saved_cfs = get_selection(system_cf[i], false, true, null);
            updateSelect(vers, sel, system_cf[i], merging, anyval);
            restoreSelection(system_cf[i], saved_cfs);
    	}
   }
}
