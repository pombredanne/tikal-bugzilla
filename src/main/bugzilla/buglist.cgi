...
my $printTemplate = $cgi->param('printTemplate') ? 1 : 0;
my $customTemplate = "list/list";
if (defined $printTemplate && $printTemplate == 1) {
		$customTemplate = "list/print-list";
}

# Determine the format in which the user would like to receive the output.
# Uses the default format if the user did not specify an output format;
# otherwise validates the user's choice against the list of available formats.
my $format = $template->get_format($customTemplate, scalar $cgi->param('format'),
...
while (my @row = $buglist_sth->fetchrow_array()) {
    ...
    $bug->{'has_subtasks'} = Bugzilla::Bug->bug_has_subtasks($bug->{'bug_id'});
...
if ($dotweak && scalar @bugs) {
    ...
    $vars->{'entities'} = get_legal_field_values('entity');
    ...
    if (scalar(@products) == 1) {
        ...
        $vars->{'targetmilestones'} = [map($_->name, @{$product->visible_milestones})]
