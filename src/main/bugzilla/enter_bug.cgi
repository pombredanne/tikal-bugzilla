...
if (scalar(@{$product->visible_components}) == 1) {
    # Only one component; just pick it.
    $cgi->param('component', $product->visible_components->[0]->name);
}
...
if (Bugzilla->params->{'usesubtasks'}){
	$vars->{'entity'} = Bugzilla::Entity::get_all_regular_entities_values();
} else {
	$vars->{'entity'} = Bugzilla::Entity::get_all_entities_values();
}
...
foreach my $field (@enter_bug_fields) {
    ...
    $default{$cf_name} = $vars->{$cf_name} = $cf_value ? $cf_value : $field->default_value || "";
}
...
if ($cloned_bug_id) {
    ...
    $default{'entity'}    	  = $cloned_bug->entity;
    ...
    my @keywords = split(",",$cloned_bug->keywords_list);
    $vars->{'keywords'} = \@keywords;

	my $input_cc;
    if (defined $cloned_bug->cc) {
        $input_cc  = $cloned_bug->cc;
    } else {
        $input_cc  = formvalue('cc');
    }
    my @cc_list;
    if ($cloned_bug->cc){
    	foreach my $cc (@{$cloned_bug->cc}){
	    	push @cc_list, { 'login' => $cc, 
	    					 'realname' => Bugzilla::User::login_to_realname($cc)
	    					};
	    }
    }
    
    if ($cloned_bug->reporter->id != $user->id) {
    	push @cc_list, { 'login' => $cloned_bug->reporter->login, 
    					 'realname' => $cloned_bug->reporter->name
    					};
    }
    $vars->{'cc'}         = \@cc_list;
    ...
    my $def_entity = $product->default_entity ? $product->default_entity : Bugzilla::Entity::get_default_entity();  
	$default{'entity'}		  = formvalue('entity', $def_entity);

    $vars->{'short_desc'}     = formvalue('short_desc');
    $vars->{'bug_file_loc'}   = formvalue('bug_file_loc', "http://");
    $vars->{'keywords'}       = formvalue('keywords');
    $vars->{'crm_list'}       = formvalue('crm_list');
    $vars->{'dependson'}      = formvalue('dependson');
    $vars->{'blocked'}        = formvalue('blocked');
    $vars->{'deadline'}       = formvalue('deadline');
    
    my @cc_list;
    foreach my $cc ($cgi->param('cc')){
    	push @cc_list, { 'login' => $cc, 
    					 'realname' => Bugzilla::User::login_to_realname($cc)
    					};
    }
...
# parameter.
$vars->{'version'} = [map($_->name, @{$product->released_versions})];
...
if ( Bugzilla->params->{'usetargetmilestone'} ) {
    $vars->{'target_milestone'} = [map($_->name, @{$product->visible_milestones})];
...
my $base_t = "bug/create/create";
my $template_name = $base_t;

my $byproduct = $base_t."-".$product_name;
my $byentity = $base_t."-".$default{'entity'};
my $byboth = $base_t."-".$product_name."-".$default{'entity'};
...
foreach my $ent (@{$vars->{'entity'}}){
	my $templates_by_ent = 0;
	if ($special_cur_tmpl ||
		$template->check_format($base_t."-".$product_name."-".$ent) || 
...
my $format = $template->get_format($template_name,
