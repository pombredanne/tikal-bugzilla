...
my @non_editable_fields = qw(product entity version component);
...
if (!$cgi->param('field')) {
    my @field_list = grep { !$block_list{$_->name} }
                       Bugzilla->get_fields({ is_select => 1, system_table => 0 });
...
if ($action eq 'new') {
    ...
    my $created_value = Bugzilla::Field::Choice->type($field)->create({
        value   => trim($cgi->param('value') || ''),
        sortkey => trim($cgi->param('sortkey') || ''),
        is_open => trim($cgi->param('is_open') || ''),
        isactive => trim($cgi->param('isactive') || 0),
        visibility_value_id => trim($cgi->param('visibility_value_id') || ''),
    });
...
if ($action eq 'update') {
    ...
    $value->set_isactive($cgi->param('isactive'));
