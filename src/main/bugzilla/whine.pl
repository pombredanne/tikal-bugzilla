...
#   query_fields	- the text inserted above the query fields
...
sub get_next_event {
    ...
    until (scalar keys %{$event}) {
        ...
        my ($eventid, $owner_id, $subject, $body, $query_fields) = @{$fetched};
...
#	query_fields (tokenized string of query fields)
while (my $event = get_next_event) {
	...
	my @flist = split( /,/, $event->{'query_fields'});
        ...
        my $args = {
            ...
            'author'      	=> $event->{'author'},
	    	'query_fields'	=> $event->{'query_fields'},
	    	'fields_list' 	=> \@flist,
            'recipient'   	=> $target,
            'from'        	=> $fromaddress,
        };
...
sub run_queries {
    ...
    foreach my $thisquery (@queries) {
		...
		my @searchfields;
        # Execute the saved query with selected query fields 
		if ($args->{'query_fields'} eq "") { # if query fields are not defined, run with default
        	@searchfields = qw(
            	bug_id
            	bug_severity
            	priority
        	...
        	# the list of whine query_fields
			my @queryfields = split( /,/, $args->{'query_fields'});
			foreach my $queryfield(@queryfields) {
				push (@searchfields, $queryfield);
			}
		}
