    ...
    if ($cgi->param("add_event")) {
        ...
        for my $eventid (keys %{$events}) {
            ...
            if ($cgi->param("remove_event_$eventid")) {
				...
				my $query_fields = join(',', $cgi->param("queryfields_${eventid}")) if ($cgi->param("queryfields_${eventid}"));

                trick_taint($subject) if $subject;
                trick_taint($body)    if $body;
				trick_taint($query_fields) if $query_fields;

                if ( ($subject ne $events->{$eventid}->{'subject'})
                  || ($body    ne $events->{$eventid}->{'body'})  
                  || ($query_fields ne $events->{$eventid}->{'query_fields'}) ) {

                    $sth = $dbh->prepare("UPDATE whine_events " .
                                         "SET subject=?, body=?, query_fields=?  " .
                                         "WHERE id=?");
                    $sth->execute($subject, $body, $query_fields, $eventid);
                }
...
my $queryfields = Bugzilla::Search->COLUMNS();
my @queryfields_list =  keys(%{$queryfields});
$vars->{'available_queryfields'} = \@queryfields_list;
...
sub get_events {
    ...
    my $sth = $dbh->prepare("SELECT DISTINCT id, subject, body, query_fields " .
                            "FROM whine_events " .
                            "WHERE owner_userid=?");
    $sth->execute($userid);
    while (my ($ev, $sub, $bod, $query_fields) = $sth->fetchrow_array) {
		my @query_fields_list;
		foreach my $qfield (split(/,/, $query_fields)) {
			push(@query_fields_list, $qfield);
		}
        $events->{$ev} = {
            'subject' => $sub || '',
            'body' => $bod || '',
